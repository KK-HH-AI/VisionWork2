...
### 第二阶段：构建记忆图谱（必须执行，为后续检索提供社群结构）
5. 使用 build_project_graph 读取所有已分析的 _entities.json 文件，构建全局知识图谱，进行层次社区检测（C0-C3四层）
   - project_path: 项目根目录路径
   - max_layers: 社区层次深度，默认4
   - 这一步会生成 graph.json、communities.json 和 community_tree.json，记忆图谱将展示社区结构
6. 使用 generate_community_summaries 为每个检测到的社区生成自然语言摘要（community_C1_0.md 等多份文件）
   - project_path: 项目根目录路径
   - api_url、api_key、model_name: LLM 连接信息
   - 生成的社区摘要会存入向量数据库，供 generate_canvas 检索使用

### 第三阶段：批量生成流程图
7. 使用 generate_canvas 一次性批量生成完整的流程图（节点+边+布局）
   - notes_summary: （可选）汇总所有 analyze_module 的分析笔记。如果不提供，工具将自动通过向量检索获取最相关的社区摘要。
   - user_intent: 用户的原始指令
   - 这是效率最高的方式，LLM 会在充分了解项目内容后一次性规划整个流程图

### 收尾
8. 使用 final_answer 输出最终分析报告
...
- 第一阶段收集信息，第二阶段构建记忆图谱，第三阶段用 generate_canvas 批量生成流程图
- analyze_module 需要提供 filepath, code_content, filename，且必须调用（记忆图谱依赖它）
- build_project_graph 需要提供 project_path（记忆图谱核心步骤，必须在 analyze_module 之后执行）
- generate_community_summaries 需要提供 project_path、api_url、api_key、model_name（必须在 build_project_graph 之后执行）
- generate_canvas 需要提供 user_intent（用户原始指令），notes_summary 可选（不提供时会自动检索社区摘要）
...
注意：
- 三阶段流程：收集信息 → 构建记忆图谱 → 批量生成流程图 → 收尾（final_answer）
- 如果已经完成了信息收集和流程图绘制，status 应为 "complete"
- 如果已经完成了信息收集但还未构建记忆图谱（build_project_graph + generate_community_summaries），应继续执行
- 如果还需要补充分析或调整流程图，status 应为 "continue" 并提供 new_plan
- 只输出JSON，不要包含```json```等标记
"""
...
def execute_node(state: AgentState) -> AgentState:
    ...
    for i in range(current_step, len(plan)):
        ...
        # 需要上下文信息的工具
        context_tools = {"analyze_module", "search_memory", "read_file", "generate_canvas", "build_project_graph", "generate_community_summaries"}
        ...
        try:
            ...
            if action == "generate_canvas":
                try:
                    ...
                    if generated_count > 0:
                        source_label = {
                            "community_summaries": "（基于社区摘要）",
                            "all_notes_fallback": "（基于全部笔记）",
                            "notes_summary": "",
                        }.get(source_type, "")
                        _push_event(event_queue, {
                            "type": "chat_response",
                            "message": f"已批量生成 {generated_count} 条画布指令，流程图已更新。{source_label}",
                        })
            ...
            if action == "search_memory":
                try:
                    result_data = json.loads(result_json)
                    cascade_path = result_data.get("cascade_path", [])
                    results = result_data.get("results", [])

                    if cascade_path:
                        retrieval_path = list(cascade_path)
                        state["retrieval_path"] = retrieval_path

                        print(f"[graph.py] search_memory (cascade): pushing memory_path_update with nodeIds: {retrieval_path}")
                        _push_event(event_queue, {
                            "type": "memory_path_update",
            ...
            # 8. build_project_graph：推送社区节点到记忆图谱
            if action == "build_project_graph":
                try:
                    result_data = json.loads(result_json)
                    community_counts = result_data.get("community_counts", {})

                    if community_counts:
                        memory_nodes = list(state.get("memory_graph_nodes", []))
                        memory_edges = list(state.get("memory_graph_edges", []))

                        communities_path = result_data.get("communities_path", "")
                        node_communities = {}
                        if communities_path and os.path.isfile(communities_path):
                            with open(communities_path, "r", encoding="utf-8") as f:
                                communities_data = json.load(f)
                            for node_info in communities_data.get("nodes", []):
                                node_communities[node_info["id"]] = node_info.get("communities", {})

                        community_nodes_map = {}
                        for layer_key in ["C0", "C1", "C2", "C3"]:
                            count = community_counts.get(layer_key, 0)
                            if count == 0:
                                continue
                            for comm_id in range(count):
                                node_id = f"community_{layer_key}_{comm_id}"
                                community_nodes_map[node_id] = {
                                    "id": node_id,
                                    "label": f"{layer_key}-社群{comm_id}",
                                    "group": "community",
                                    "level": layer_key,
                                    "community_id": str(comm_id),
                                }
                                if node_id not in {n["id"] for n in memory_nodes}:
                                    memory_nodes.append(community_nodes_map[node_id])

                        for parent_key, child_key in [("C0", "C1"), ("C1", "C2"), ("C2", "C3")]:
                            parent_count = community_counts.get(parent_key, 0)
                            child_count = community_counts.get(child_key, 0)
                            if parent_count == 0 or child_count == 0:
                                continue
                            for child_comm_id in range(child_count):
                                child_node_id = f"community_{child_key}_{child_comm_id}"
                                parent_comm_id = child_comm_id % parent_count if parent_count > 0 else 0
                                parent_node_id = f"community_{parent_key}_{parent_comm_id}"
                                edge = {
                                    "source": parent_node_id,
                                    "target": child_node_id,
                                    "label": "contains",
                                    "type": "community_hierarchy",
                                }
                                edge_key = f"{parent_node_id}->{child_node_id}"
                                existing_edge_keys = {f"{e['source']}->{e['target']}" for e in memory_edges}
                                if edge_key not in existing_edge_keys:
                                    memory_edges.append(edge)

                        for file_node_id, comms in node_communities.items():
                            for layer_key in ["C3", "C2", "C1", "C0"]:
                                comm_id = comms.get(layer_key, -1)
                                if comm_id >= 0:
                                    community_node_id = f"community_{layer_key}_{comm_id}"
                                    edge = {
                                    ...
                                    edge_key = f"{community_node_id}->{file_node_id}"
                                    existing_edge_keys = {f"{e['source']}->{e['target']}" for e in memory_edges}
                                    if edge_key not in existing_edge_keys:
                                        memory_edges.append(edge)
                                    break
                        ...
                        state["memory_graph_nodes"] = memory_nodes
                        state["memory_graph_edges"] = memory_edges

                        _push_event(event_queue, {
                            "type": "memory_graph",
                            "nodes": list(memory_nodes),
                            "edges": list(memory_edges),
                            "memory_dir": state.get("memory_dir", ""),
                        })

                        layer_summary = ", ".join(
                            f"{k}:{v}个社群" for k, v in community_counts.items() if v > 0
                        )
                        _push_event(event_queue, {
                            "type": "chat_response",
                            "message": (
                                f"记忆图谱已构建：{result_data.get('node_count', 0)}个实体节点, "
                                f"{result_data.get('edge_count', 0)}条关系边, "
                                f"社区层次: {layer_summary}"
                            ),
                        })
                except Exception as e:
                    logger.warning(f"[graph.py] build_project_graph post-processing failed: {e}")

            # 9. generate_community_summaries：标注社区摘要路径
            if action == "generate_community_summaries":
                try:
                    result_data = json.loads(result_json)
                    summary_count = result_data.get("summary_count", 0)
                    communities_dir = result_data.get("communities_dir", "")

                    if communities_dir and os.path.isdir(communities_dir):
                        memory_nodes = list(state.get("memory_graph_nodes", []))
                        for filename in os.listdir(communities_dir):
                            ...
                            node_id = f"community_{level}_{comm_id}"

                            summary_path = os.path.join(communities_dir, filename)
                            for node in memory_nodes:
                                if node["id"] == node_id:
                                    node["path"] = summary_path
                    ...
                    _push_event(event_queue, {
                        "type": "chat_response",
                        "message": (
                            f"社区摘要已生成：{summary_count} 份 markdown 文件, "
                            f"存储在 {communities_dir}"
                        ),
                    })
                except Exception as e:
                    logger.warning(f"[graph.py] generate_community_summaries post-processing failed: {e}")
