...
6. 使用 canvas_read 读取当前画布状态（节点和边），了解已有的流程图结构
7. 使用 draw_relation 的 update_node/remove_node/remove_edge 操作来修改已有的流程图
8. 最后使用 final_answer 输出最终分析报告

## 当前画布状态
{canvas_context}

## 修改流程图指南
- 如果用户要求修改已有的流程图节点或边，先使用 canvas_read 了解当前画布状态
- 使用 draw_relation 的 update_node 操作修改节点标签和属性
- 使用 draw_relation 的 remove_node 操作删除节点（同时会删除相关边）
- 使用 draw_relation 的 remove_edge 操作删除边
- 使用 draw_relation 的 add_edge 操作（默认）添加新边
...
- draw_relation add_edge 需要提供 source_id, target_id, label 参数
- draw_relation update_node 需要提供 node_id 以及 new_label, new_type, new_group
